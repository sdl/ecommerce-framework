...
public class FredhopperPromotion implements Promotion, Editable {
    ...
    public static Promotion build(Theme theme, FredhopperLinkManager linkManager, List<Product> products, String editUrl) {
        if ( theme.getStaticContent() != null ) {
            String type =  theme.getStaticContent().getType().value();
            if ( type.equals("image") ) {
                return new ImagePromotion(theme, linkManager, editUrl);
            }
            else if (type.equals("image-map") ) {
                return new FredhopperImageMapPromotion(theme, linkManager, editUrl);
            }
            else if ( type.equals("text") ) {
                return new TextPromotion(theme, linkManager, editUrl);
            }
        }
        else {
            // Product promotion
    ...
    protected String getPromotionLink(ContentLinkBase contentLink, FredhopperLinkManager linkManager) {

        if ( contentLink.getType().value().equals("catalog") ) {

            // Get the fh_location and fh_reftheme
            // to start with use only location
            //

            // TODO: Have a link mgmt service that can convert between fh links and SEO links
            ...
            while ( tokenizer.hasMoreTokens() ) {
                ...
                if ( token.equals("fh_location") ) {
                    ...
                    return linkManager.convertToSEOLink(locationLink);
                }
            }
            return null;
